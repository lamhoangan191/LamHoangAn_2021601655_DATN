...
$stmt = $conn->prepare("SELECT trangthai, COUNT(*) c FROM donnhannuoi WHERE user_id=? GROUP BY trangthai");
...
$sql = "SELECT d.*, t.tentc, t.anh
        FROM donnhannuoi d
        JOIN thucung_coso t ON d.petcs_id = t.petcs_id
        WHERE d.user_id = ? AND d.trangthai = ?
...
<head>
  ...
  <style>
    ...
    .pet-thumb {
      ...
      margin: 8px auto; border-radius: 10px; background:#f6f6f6;
    }
    .no-img {
      width:100px;height:100px;display:flex;align-items:center;justify-content:center;
      background:#f1f1f1;border-radius:10px;color:#888;font-weight:600;
      margin:8px auto;
    }
...
<div class="container mt-3 mb-5 h-50">
  ...
  <?php while($row = $list->fetch_assoc()): ?>
    <div class="don-card">
      <!-- Ô 1: ảnh + tên thú cưng -->
      <div class="don-section text-centers">
        <?php
      ...
      <div class="don-section text-center border-start">
        ...
        <?php else: ?>
          ...
          <div class="status-approved badge bg-success text-white fs-6">
            <i class="fa-solid fa-circle-check"></i> Đã duyệt (<?php echo $ngayduyet ?>)
          </div>
        <?php endif; ?>
      </div>
      <!-- Ô 3: Nút -->
      <div class="don-section border-start">
        <a href="chitiet_donnhannuoi_kh.php?id=<?php echo $row['donnn_id']; ?>" class="btn btn-info mb-2 fw-bold w-50">Chi tiết</a>
        <?php if ($row['trangthai'] === 'chờ duyệt'): ?>
        <a href="xoa_donnhannuoi_kh.php?id=<?php echo $row['donnn_id']; ?>" onclick="return confirm('Bạn chắc chắn muốn xoá?')" class="btn btn-danger fw-bold w-50">Xóa</a>
        <?php endif; ?>
      </div>
    </div>
  <?php endwhile; ?>
